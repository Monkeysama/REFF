...
window.addEventListener('pagehide', removeInputFocusReporter, { once: true });
...
function App() {
  ...
  return <main className="plugin-page"><section className="panel"><h2>运行信息</h2>{snapshot ? <dl>{[['游戏', snapshot.gameName], ['REFramework', snapshot.reframeworkVersion], ['REFF 运行时间', `${snapshot.uptimeSeconds} 秒`], ['刷新次数', String(snapshot.refreshCount)]].map(([label, value]) => <div className="row" key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl> : <p>等待 REFF 服务</p>}<button disabled={!ready} onClick={() => void refresh()}>调用 REFF 刷新</button><p id="error" /></section><section className="panel section-panel"><h2>输入法测试</h2><input className="text-input" value={inputText} onChange={event => setInputText(event.target.value)} placeholder="请输入中文或英文" /><p className="input-preview">当前输入：{inputText || '暂无内容'}</p></section><section className="panel section-panel"><h2>玩家血量</h2>{snapshot?.hp.available ? <><div className="hp-value">当前：{snapshot.hp.current?.toFixed(0)} / {snapshot.hp.max?.toFixed(0)}（{snapshot.hp.percent?.toFixed(1)}%）</div><div className="hp-control"><label>调整血量：{hpPercent}% <input type="range" min="0" max="100" value={hpPercent} disabled={!snapshot.hp.adjustable} onInput={event => setHpPercent(Number((event.target as HTMLInputElement).value))} /></label><button disabled={!ready || !snapshot.hp.adjustable} onClick={() => void setHp()}>应用血量</button></div></> : <p>当前游戏不支持读取玩家血量</p>}</section></main>;
}
