...
1. REFramework 加载 DLL，检查运行时接口与已支持的游戏/渲染器。
...
Windows 消息拦截不能代表 Raw Input、轮询按键或 XInput 已被屏蔽。Wilds 的实际输入路径需要验证；如需游戏适配，封装在 adapter 中。中文 IME 包含组合文本、候选窗位置及焦点恢复，不能只验证英文字符输入。
